...
from eg3d.projector.PTI.training.coaches.single_image_coach import SingleImageCoach


def run_PTI(image_name: str,
            /,
    ...
    image_path = f"../../../../gan_inversion/data/{image_name}.png"
    name = image_name # os.path.basename(image_path)[:-4]
    w_path = f'../../projector_out/{name}_{latent_space}/{name}_{latent_space}.npy'
    c_path = f'../../../../gan_inversion/data/{name}.npy'
    if len(glob.glob(f'./checkpoints/*_{name}_{latent_space}.pth')) == 0 and os.path.exists(w_path):
        coach.train(image_path = image_path, w_path=w_path,c_path = c_path)
