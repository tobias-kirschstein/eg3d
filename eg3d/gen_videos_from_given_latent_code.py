...
# Necessary to override KMP_DUPLICATE_LIB_OK due to the following error:
# OMP: Error #15: Initializing libiomp5md.dll, but found libiomp5md.dll already initialized.
# OMP: Hint This means that multiple copies of the OpenMP runtime have been linked into the program.
# That is dangerous, since it can degrade performance or cause incorrect results.
# The best thing to do is to ensure that only a single OpenMP runtime is linked into the process, e.g. by avoiding
# static linking of the OpenMP runtime in any library. As an unsafe, unsupported, undocumented workaround you can set
# the environment variable KMP_DUPLICATE_LIB_OK=TRUE to allow the program to continue to execute, but that may cause
...
def gen_interp_video(G, latent, mp4: str, w_frames=60 * 4, kind='cubic', grid_dims=(1, 1),
                     ...
                     gen_shapes=False, device=torch.device('cuda'),
                     **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
...
def generate_images(
        ...
        noise_buffers: Optional[Dict[str, torch.Tensor]] = None,
):
    ...
    if noise_buffers is not None:
        for k, noise_buffer in noise_buffers.items():
